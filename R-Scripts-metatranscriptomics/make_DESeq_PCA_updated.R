...
pcplot <- ggplot(PCAplot, aes(PC1, PC2, color=conditions)) +
  geom_point(size=2.5) +
    ##### Adding ellipses using ggforce ######
    ggforce::geom_mark_ellipse(aes(fill = conditions,
                               color = conditions)) +
    theme(legend.position = 'bottom') + 
    geom_label_repel(aes(label=completeCondition2$samples), fontface = 'bold', size=3) + 
    xlim(-12,5) + ylim(-7.5,6) +
    #geom_text(aes(label=name), hjust=1, vjust=-1) +
    ggtitle("PCA Plot of control vs. experimental organism data") +
    theme(legend.position = "bottom") +  theme(legend.title=element_text(size=10, face = 'bold')) +
