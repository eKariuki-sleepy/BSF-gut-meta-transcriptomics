...
option_list = list(
  ...
  make_option(c("-O", "--out"), type="character", default="combined_graph_21.pdf",
              help="output file name [default= %default]", metavar="character"),
  make_option(c("-D", "--directory"), type="character", default="A:/Analy2is/R_analysis/counts/Organism",
              help="working directory location", metavar="character"),
  make_option(c("-N", "--number"), type="integer", default=21,
...
org_relative_ggplot <- ggplot(full_table_top30, aes(x = variable, y = value, fill = Genus)) +
  ...
  theme(legend.position = "none", text=element_text(face = "bold", size=11),
...
org_absolute_ggplot <- ggplot(full_table_top30, aes(x = variable, y = value, fill = Genus)) +
  ...
  scale_fill_manual(breaks = Genus2, values = Cb64k) +
  theme(legend.position = "bottom", text=element_text(face = "bold", size=11),
        axis.text.x = element_text(angle=90, vjust=1)) +
