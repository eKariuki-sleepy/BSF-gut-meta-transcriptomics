...
option_list = list(
  make_option(c("-I", "--input"), type="character", default="A:/Analy2is/R_analysis/counts/Subsystems",
...
if (is.null(opt$raw_counts) == FALSE) {
  ...
  colnames(data3) <- colnames(l1_table) # Change column names
  data3                                 # Print updated data
  
  
  l1_table <- rbind(l1_table, data3["OTHER",])
}
