...
option_list = list(
  ...
  make_option(c("-O", "--out"), type="character", default="combined_func_graph_20.pdf",
              help="output file name [default= %default]", metavar="character"),
  make_option(c("-D", "--directory"), type="character", default="A:/Analy2is/R_analysis/counts/Function",
...
control_table_trimmed_m <- melt(cbind(control_table_trimmed,
      Function = rownames(control_table_trimmed)), id.vars = c('Function'))

exp_table_trimmed_m <- melt(cbind(exp_table_trimmed,
      Function = rownames(exp_table_trimmed)), id.vars = c('Function'))
...
control_table_filtered_m <- melt(cbind(control_table_filtered,
     Function = rownames(control_table_filtered)), id.vars = c('Function'))

exp_table_filtered_m <- melt(cbind(exp_table_filtered,
     Function = rownames(exp_table_filtered)), id.vars = c('Function'))
...
# plotting
Function2 = full_table_top30$Function
Function2 = Function2[Function2 != "Filler"]

relative_func_ggplot <- ggplot(full_table_top30, aes(x = variable, y = value, fill = Function)) +
  geom_bar(position = "fill", stat = "identity") +
  scale_fill_manual(breaks = Function2, values = Cb64k) +
...
relative_func_ggplot

func_absolute_ggplot <- ggplot(full_table_top30, aes(x = variable, y = value, fill = Function)) +
...
func_absolute_ggplot

# saving and finishing up
cat ("Saving Plot as ", save_filename, " now.\n")
pdf(file = save_filename, width=9, height=12)
grid.arrange(relative_func_ggplot, func_absolute_ggplot, ncol = 1)
dev.off()
